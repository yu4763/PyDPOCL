...
if __name__ == '__main__':
	...
	domain_file = 'Ground_Compiler_Library//domains/play_detective_domain.pddl'

	# Problem files
	# 1] 1 agent, 1 car, 1 airplane, 2 locations
	problem_file_1 = 'Ground_Compiler_Library//domains/play_detective_problem_a.pddl'
	# 2] 2 agents, 1 car, 1 airplane, 2 locations, 1 goal
	problem_file_2 = 'Ground_Compiler_Library//domains/play_detective_problem_b.pddl'
	...
	for prob in problems:
		...
		var = str(out)
		length = min(k, len(out))

		for j in range(length):
			for step in out[j].OrderingGraph.topoSort():
				print('\t\t{}\n'.format(str(step)))
				print()
			print('-------------------------------------------------------------------------')

		print("number of plans: " + str(length))